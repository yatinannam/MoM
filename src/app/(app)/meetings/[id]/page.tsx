  ...
  return (
    <div className="p-8 md:p-10 max-w-4xl mx-auto w-full">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-10 gap-4">
        <div>
          <h2 className="text-2xl font-semibold text-slate-900 dark:text-white tracking-tight mb-2">{meeting.title}</h2>
        ...
        <Button variant="outline" onClick={copy} className="h-10 rounded-xl px-4 text-[13px] font-semibold border-slate-200 shadow-sm dark:border-slate-800">
      ...
      <div className="flex gap-1 mb-6 border-b border-slate-200 dark:border-slate-800">
        {(["mom", "summary"] as const).map(t => (
          <button 
            key={t} 
            onClick={() => setTab(t)} 
            className={`px-5 py-3 text-[14px] font-medium border-b-2 transition-colors ${tab === t ? 'border-blue-600 text-blue-600 dark:border-blue-500 dark:text-blue-400' : 'border-transparent text-slate-500 hover:text-slate-800 dark:hover:text-slate-300'}`}
          >
      ...
      <div className="bg-white dark:bg-slate-950 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm p-8 min-h-[400px]">
        {tab === "summary" ? (
          <p className="text-[14.5px] leading-relaxed text-slate-700 dark:text-slate-300">{meeting.summary}</p>
        ) : (
